...
export function FlightDetails() {
  ...
  //   // Envoie la réservation au backend (requiert token)
  //   try {
  //     const token = localStorage.getItem("token");
  //     const payload = {
  //       airline: flight.airline,
  //       origin: flight.origin || flight.departureAirport || flight.depart,
  //       destination:
  //         flight.destination || flight.arrivalAirport || flight.arrivee,
  //       departureDate: date,
  //       departureTime: flight.departureTime || flight.heureDepart,
  //       arrivalDate: null,
  //       arrivalTime: flight.arrivalTime || flight.heureArrivee,
  //       price: flight.price || 0,
  //       nbPassagers: 1,
  //       classe: flight.cabinClass || "Economy",
  //     };
  ...
  //     // Affiche une confirmation et redirige vers le dashboard utilisateur
  //     alert(
  //       "Réservation enregistrée (id: " + data?.reservation?.rechercheId + ")",
  //     );
  //     navigate("/user/favorites");
  //   } catch (e) {
  //     console.error(e);
  //     alert(e.message || "Impossible d'enregistrer la réservation");
  //   }
  // };
  const handleBookClick = () => {
    // Récupère les variables d'environnement depuis .env
    // eslint-disable-next-line no-undef
    const marker = process.env.REACT_APP_TRAVELPAYOUTS_MARKER;
    // eslint-disable-next-line no-undef
    const baseUrl = process.env.REACT_APP_TRAVELPAYOUTS_BOOKING_URL;

    // Construit le lien de réservation avec les paramètres du vol
    const travelPayoutsUrl =
      `${baseUrl}?marker=${marker}` +
      `&origin=${flight.origin}` +
      `&destination=${flight.destination}` +
      `&date=${date}` +
      `&passengers=1`;

    // Redirection vers le lien TravelPayouts dans une nouvelle fenêtre
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1">
                ...
                {/* Extraire le nom de l'aéroport et le code IATA depuis flight.origin */}
